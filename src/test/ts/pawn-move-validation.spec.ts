...
export class TestBlackPawnMoves {
    ...
    testPawnCanMoveForward() {
        ...
        const singleForward: Move = { from: positions.A7, to: positions.A6, isValid: true };
    ...
    testPawnCannotMoveBackward() {
        putPiece(chessboard, positions.A7, pieces.blackPawn);
        const singleForward: Move = { from: positions.A7, to: positions.A8, isValid: true };
    ...
    testPawnInitialMove() {
        putPiece(chessboard, positions.A7, pieces.blackPawn);
        const doubleForward: Move = { from: positions.A7, to: positions.A5, isValid: true };
    ...
    testCannotMoveTwoSquaresIfAlreadyMoved() {
        ...
        const doubleForward: Move = { from: positions.C6, to: positions.C4, isValid: true };
    ...
    testCannotMoveThreeSquares() {
        putPiece(chessboard, positions.C6, pieces.blackPawn);
        const tripleForward: Move = { from: positions.A7, to: positions.A4, isValid: true };
    ...
    testPawnCannotCaptureEmptySquare() {
        putPiece(chessboard, positions.A7, pieces.blackPawn);
        const diagonalCapture: Move = { from: positions.A7, to: positions.B6, isValid: true };
